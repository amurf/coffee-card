...
import { getCardById, getStoreByName, redeem, createPendingRedemption, commitRedemption } from "@coffee-card/backend";
  ...
  try {
    if (actionType === "add_stamps") {
      ...
      if (pending) {
        ...
        return json<ActionData>({ error: "Unable to claim milestone. Not enough stamps or already claimed." }, { status: 400 });
      }
    }
    ...
    if (!card) {
      return json<ActionData>({ error: "Card not found in database" }, { status: 404 });
    }
    const store = await getStoreByName(card.storeName);
    if (!store) {
      return json<ActionData>({ error: "Store details not found" }, { status: 404 });
    }
...
export default function ScanPage() {
  ...
  const tabs = [
    ...
    { id: "manual-entry", content: "Manual Entry", panelID: "manual-entry-content" },
  ];
      ...
      const timer = setTimeout(() => {
        ...
        scanner.render(
          (decodedText) => {
            ...
            scanner.clear();
            submit({ cardId: decodedText, _action: "lookup" }, { method: "post" });
          },
          (error) => {}
        );
  ...
  const handleAddStamps = () => {
    ...
    if (stampsEarned > 0) {
      submit({ cardId: actionData!.card!.cardId, _action: "add_stamps", stamps: stampsEarned.toString() }, { method: "post" });
      setInputValue("");
    }
  };

  const handleClaimReward = (milestoneId: string) => {
    submit({ cardId: actionData!.card!.cardId, _action: "claim_reward", milestoneId }, { method: "post" });
  };
      ...
      <BlockStack gap="500">
        <Layout>
          <Layout.Section>
            ...
            {actionData?.error && (
              <Banner tone="critical" title="Error" onDismiss={() => submit({ cardId: card?.cardId || "", _action: "lookup" }, { method: "post" })}>
            ...
            {!card ? (
                ...
                <BlockStack gap="400">
                  ...
                  {selectedTab === 1 && (
                    <BlockStack gap="400">
                      ...
                      />
                      <Button onClick={handleManualSubmit} loading={isSubmitting} disabled={!manualId} variant="primary">
                        Look up Card
                      </Button>
                    </BlockStack>
                  )}
              ...
              <BlockStack gap="500">
                <Card>
                  <BlockStack gap="400">
                    <InlineStack align="space-between">
                      <Text as="h2" variant="headingLg">Card Found</Text>
                ...
                <Card>
                  <BlockStack gap="400">
                     <Text as="h2" variant="headingMd">Add Stamps</Text>
                     <Grid>
                        <Grid.Cell columnSpan={{xs: 6, sm: 4, md: 4, lg: 8, xl: 8}}>
                          <TextField
                            label={earningRule?.type === "SPEND_AMOUNT" ? `Purchase Amount ($) - Earns 1 stamp per $${earningRule.amountPerStamp}` : "Number of items purchased"}
                            value={inputValue}
                        ...
                        <Grid.Cell columnSpan={{xs: 6, sm: 2, md: 2, lg: 4, xl: 4}}>
                          <div style={{ marginTop: '24px' }}>
                             <Button onClick={handleAddStamps} loading={isSubmitting} variant="primary" fullWidth>Grant Stamps</Button>
                          </div>
                        </Grid.Cell>
                     </Grid>
                  </BlockStack>
                ...
                <Card>
                  <BlockStack gap="400">
                    <Text as="h2" variant="headingMd">Available Perks</Text>
                    {sortedMilestones.length === 0 ? (
                      <Text as="p" tone="subdued">No rewards configured for this store.</Text>
                    ) : (
                      <BlockStack gap="300">
                            ...
                            <div key={milestone.id}>
                              <InlineStack align="space-between" blockAlign="center">
                                ...
                                <Button 
                                  ...
                                  variant={!isClaimed && !isLocked ? "primary" : undefined}
                                  loading={isSubmitting}
                                >
                                  {isClaimed ? "Already Claimed" : "Claim"}
                                </Button>
                              </InlineStack>
                              <div style={{ marginTop: '1rem', marginBottom: '1rem' }}><Divider /></div>
                            </div>
                          );
                        })}
                      </BlockStack>
